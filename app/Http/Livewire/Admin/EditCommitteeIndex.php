...
class EditCommitteeIndex extends Component
{
    use WithFileUploads;
    public $imagen;
    public $form;
    ...
    public function mount(Comites $form)
    {

        $this->form = $form;

        $this->orden = Comites::where('estado',1)
        ->orderBy('orden', 'asc')
        ->get();
    }
    public function render()
    {
        return view('livewire.admin.edit-committee-index');
    }
    public function update()
    {
        ...
        if($this->form->orden!=Comites::whereId($this->form->id)->value('orden'))
        {
