<div>
    <div class="card">
        <div class="card-header">
            <h5><strong>Todas las Inducciones</strong></h5>
            <div class="card-body">
                <div class="row">
                    <div class="col-8">
                        <x-modal-induction-new />
                    </div>
                    <div class="col-2">
                            ...
                            @else
                                <tr>
                                    ...
                                    <td><input style="min-width:25rem" type="text" value="{{ $form->adjunto }}"
                                            disabled></td>
                                     <td width="10px">
                                        <livewire:admin.edit-inductions-index :form="$form" :wire:key="$form->id">
                                    </td>
...
@push('js')
    ...
    <script>
        Livewire.on('des_hau', posId => {
            Swal.fire({
                title: '¿Estás seguro(a) de deshabilitar esta Inducción?',
                ...
                if (result.isConfirmed) {
                    Livewire.emitTo('admin.inductions-index', 'deshabilitar', posId);
                    Swal.fire(
                        'Proceso Ejecutado!',
                        'Esta Inducción ha sido deshabilitada.',
                        'success'
                    )
                }
            })
    ...
    <script>
        Livewire.on('hab', posId => {
            Swal.fire({
                title: '¿Estás seguro(a) de esta Inducción?',
                text: "¡No podrás revertir esta acción!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#FE3EB2',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire(
                        '¡Habilitada!',
                        'Esta Inducción ha sido habilitada.',
                        'success'
                    )
    ...
    <script>
        Livewire.on('delet', posId => {
            Swal.fire({
                title: '¿Estás seguro(a) de eliminar esta Inducción?',
                text: "¡No podrás revertir esta acción!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#FE3EB2',
                ...
                if (result.isConfirmed) {
                    Livewire.emitTo('admin.inductions-index', 'delete', posId);
                    Swal.fire(
                        '¡Eliminada!',
                        'Esta Inducción ha sido eliminada.',
                        'success'
                    )
