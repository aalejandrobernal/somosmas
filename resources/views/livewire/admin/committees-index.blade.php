<div>
    <div class="card">
        <div class="card-header">
            <h5><strong>Todos los Comités</strong></h5>
            <div class="card-body">
                <div class="row">
                    <div class="col-8">
                        <x-modal-committee-new />
                    </div>
                    <div class="col-2">
                            ...
                            @else
                                <tr>
                                    ...
                                    <td><input style="min-width:25rem" type="text" value="{{ $form->adjunto }}"
                                            disabled></td>
                                     <td width="10px">
                                        <livewire:admin.edit-committee-index :form="$form" :wire:key="$form->id">
                                    </td>
...
@push('js')
    ...
    <script>
        Livewire.on('des_hau', posId => {
            Swal.fire({
                title: '¿Estás seguro(a) de deshabilitar este comité?',
                ...
                if (result.isConfirmed) {
                    Livewire.emitTo('admin.committees-index', 'deshabilitar', posId);
                    Swal.fire(
                        'Proceso Ejecutado!',
                        'Este comité ha sido deshabilitada.',
                        'success'
                    )
                }
            })
    ...
    <script>
        Livewire.on('hab', posId => {
            Swal.fire({
                title: '¿Estás seguro(a) de este comité?',
                text: "¡No podrás revertir esta acción!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#FE3EB2',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire(
                        '¡Habilitada!',
                        'Este comité ha sido habilitada.',
                        'success'
                    )
    ...
    <script>
        Livewire.on('delet', posId => {
            Swal.fire({
                title: '¿Estás seguro(a) de eliminar este comité?',
                text: "¡No podrás revertir esta acción!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#FE3EB2',
                ...
                if (result.isConfirmed) {
                    Livewire.emitTo('admin.committees-index', 'delete', posId);
                    Swal.fire(
                        '¡Eliminada!',
                        'Este comité ha sido eliminada.',
                        'success'
                    )
